...
- CoWoS / HBM advanced packaging (the step that turns high-bandwidth memory + logic dies into the actual AI monsters everyone is buying).
...
- advanced chips are literally called “strategic technologies,”  
- export controls and IP wars are the norm,  
...
- **Literature map sketched**  
  ...
  - decarb + multi-objective optimization (carbon pricing, emission factors),
...
- There will be **regions on the frontier where resilience and decarb align** (e.g. moving volume to cleaner but also more diversified sites).
...
- **Objectives:**
  ...
  - \( f_2 \): resilience metric (e.g. service loss / time to recovery aggregated over scenarios),
  - \( f_3 \): life-cycle CO₂e of packaging + logistics.
- **Solution idea:**
  - Use a multi-objective heuristic (e.g. NSGA-II/III or related) to generate candidate designs.
  - For each design, run a set of discrete-event / Monte-Carlo simulations across disruption scenarios.
- **Emissions accounting:**
  - freight emissions: GLEC / ISO-style \( \text{CO₂} = d \times w \times EF_{mode} \),
...
This is all still in motion, but the spine is there. As the thesis matures, this page will turn into the running log of what changed and what actually worked.
